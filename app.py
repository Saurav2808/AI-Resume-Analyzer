...
from services.pdf_saurav import extract_pdf_text
...
from chains.evalution_chain import evalute_answer_chain
...
if not api_key:
    st.error("API Key not found. Please verify your .env file contains GOOGLE_API_KEY or GEMINI_API_KEY.")
    st.stop()

# Strip any accidental whitespace or quotes from the key
api_key = api_key.strip().strip('"').strip("'")

# gemini-1.5-flash and gemini-2.5-flash were both retired by Google —
# using gemini-3.6-flash, the currently supported model as of mid-2026
...
for key in ("resume_text", "ats", "skills", "source_file", "current_question", "latest_evaluation", "rewritten_resume", "html_resume"):
...
def run_analysis(cleaned_text, llm):
    ats_chain = analyze_resume_chain(cleaned_text, llm)
    skills_chain = extract_skills_from_resume_chain(cleaned_text, llm)
    parallel_chain = RunnableParallel(ats=ats_chain, skills=skills_chain)
...
def render_questions():
    ...
    if st.button("Start Mock Interview"):
        ...
        # NOTE: generate_question_chain's real parameter is spelled "roll"
        # (a typo in chains/interview_chain.py), not "role". Renamed here to
        # match — consider fixing the typo at its source for clarity.
        question = generate_question_chain(
            name=st.session_state.skills.candidate_name,
            roll=st.session_state.skills.target_role,
    ...
    if st.session_state.current_question:
        ...
        if st.button("Submit Answer"):
            evaluation = evalute_answer_chain(
...
def render_rewrite_resume():
    ...
    if st.button("Rewrite Resume"):
        if job_role and job_description:
            ...
            st.session_state.html_resume = None
        else:
    ...
    with st.sidebar:
        ...
        if uploaded_file and uploaded_file.name != st.session_state.source_file:
            with st.spinner("Extracting Resume..."):
                ...
                if not cleaned_text:
                    st.error("Failed to extract text from the resume. Please ensure the file is a valid PDF or DOCX.")
                    st.stop()
